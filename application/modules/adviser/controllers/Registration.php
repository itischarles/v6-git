...
/**
 * Adviser Registration Controller
 *
 * @author Wajira Weerasinghe
 *
 * IMPORTANT NOTE: This controller require no authentication. So this is moved 
 * to a different class other than all other adviser related fucntions.
 * 
 * 
 * 
 */
class Registration extends MY_Controller {
    ...
    /**
     * Self Register IFA ----------------------------------------------
     * Show self registration form and let advisers to register themselves.
     * Firms and Firm Networks are also created.
     * New Adviser is created with  a new user record as well.
     */
    function selfRegister() {
        ...
        if ($this->input->post('faSelfRegister')) {
            ...
            if (trim($networkID) == '_NEW_' && trim($firmID) == '_NEW_') {
                ...
                $this->form_validation->set_rules('networkFCANo', 'Firm Netwrok FCA Ref. Number', 'trim|required|is_unique[ifa_firm_network.fcaNumber]');
            ...
            if (trim($firmID) == '_NEW_') {
                ...
                $this->form_validation->set_rules('firmFCANo', 'Firm FCA Ref. Number', 'trim|required|is_unique[ifa_firm.fcaNumber]');
            ...
            $this->form_validation->set_rules('individualFcaNumber', 'Personal FCA Ref. Number', 'trim|required|is_unique[ifa.individualFcaNumber]');
            ...
            if ($validForm) {
                ...
                if (( $firmID > 0 ) && (trim($firmID) != '_NEW_' ) && (trim($firmID) != '' )) {
                 ...
                 //Get the appropriate role id from role table by giving a code for 'Normal Adviser'
                ...
                //Now add default Normal adviser role to this adviser
                $arcontent['ifaID']= $newAdviserID;
                $arcontent['ifaRoleID']= $roleID;
                 
                //Add this adviser to default 'Normal Advisor' Role.
                $retVal = $this->adviser_adviser_role_accessor->addAdviserRole($arcontent);
                
                //If a new firm then this advisor becomes the Super Adviser for this firm by default
                if($newFirmAdding)
                {
                    $roleRet = $this->adviser_role_accessor->getIDfromCode('AS');
                    if ($roleRet->success) {
                    ...
                    //Add this adviser to 'default 'Normal Advisor''Super Adviser' Role.
                    $retVal = $this->adviser_adviser_role_accessor->addAdviserRole($arcontent);
                }
            ...
            if ($validForm && trim($allErrors) == '') { //Excellent ... all gone as plannned.... 
                $this->session->set_flashdata('message', 'Adviser self registration successful. <br>Please login to adviser module using the credentials you specified.' . $msg);
     ...
     /**
     * searchFirms ----------------------------------------------
     * Search list of firms based on text input.
     * return is a list of matching firms
     * 
     */
    function searchFirms() {
     ...
     /**
     * searchNetworks  ----------------------------------------------
     * Search list of firm networks based on text input.
     * return is a list of matching networks
     * 
     */
    ...
    /**
     * Password check callback function for cusom validation -----------------
     * Search list of firm networks based on text input.
     * return is a list of matching networks
     * 
     */

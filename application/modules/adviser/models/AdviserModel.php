...
class AdviserModel extends CI_Model {
    ...
    /**
     * is_paraplannerAdviser
     * check if this adviser has a role of paraplanner
     * @param int $adviserID
     * @return boolean
     */
    function is_paraplannerAdviser($adviserID){
        $adviserRoles = $this->getAdviserRoles($adviserID);
        $resp = array();
        
        if(!empty($adviserRoles)){
            foreach($adviserRoles as $key=>$adviserRole){
                if(strtoupper($adviserRole->ifaRoleCode) == "AP"){
                    $resp[] = 1;
                }
            }
        }
     ...
     /**
     * is_normalAdviser
     * check if this adviser a regular/normal adviser
     * @param int $adviserID
     * @return boolean
     */
    function is_normalAdviser($adviserID){
        $adviserRoles = $this->getAdviserRoles($adviserID);
        $resp = array();
        
        if(!empty($adviserRoles)){
            foreach($adviserRoles as $key=>$adviserRole){
                if(strtoupper($adviserRole->ifaRoleCode) == "AN"){
                    $resp[] = 1;
                }
            }
        }
